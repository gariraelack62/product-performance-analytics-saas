...
/*
=====================================================================================
gold_fact_advcanced_activation
=====================================================================================

Context
- Many accounts sign up, but only some reach real product usage.
- This table defines a simple, explanation activation signal
...
WITH accounts AS (
    ...
    WHERE e.HasDealContext = TRUE
        AND CAST(e.EventsDate AS DATE)
            BETWEEN a.CreationDate AND (a.CreationDate + INTERVAL 14 DAY)
    GROUP BY 1
    ),
...
--First observed deal activity after signup
    ...
    CASE
        WHEN e.FirstCoreEvents IS NOT NULL
            AND d.FirstDealDate IS NOT NULL
            THEN TRUE
            ELSE FALSE
    END AS IsActivated,
    ...
    CASE
        WHEN e.FirstCoreEventDate IS NOT NULL
            AND d.FirstDealDate IS NOT NULL
        THEN DATE_DIFF('day', a.CreatedDate, GREATEST(e.FirstCoreEventDate, FirstDealDate))
        ELSE NULL
    END AS TimeToValueDays,
            

-- Diagnostic counts used to for analysis and segmentation
...
print ( "Created gold__fact_advanced_activation table")
